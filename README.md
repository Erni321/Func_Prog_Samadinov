...
1)Climbing stairs:
  Дан алгоритм для подсчета количества уникальных способов подняться по лестнице из n ступенек, где за один шаг можно подняться на 1 или 2 ступеньки. Задача сводится к последовательности Фибоначчи:
  F(1) = 1 (только: 1)
  F(2) = 2 (1+1, 2)
